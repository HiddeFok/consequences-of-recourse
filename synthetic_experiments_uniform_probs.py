...
from utils.experiment import do_synthetic_experiment_acceptence_probs, save_synthetic_experiment_data_acceptence_probs
...
if __name__ == "__main__":
    SEED = 124
    TEST_SIZE = 1000
    UNIFORM_PROBS = np.linspace(0, 1, num=100)
    set_seed(SEED)
    ...
    checkpoint_dir = f"checkpoints/{args.data_set}_uniform_probs"
    ...
    result_dict = do_synthetic_experiment_acceptence_probs(
        X_train, X_test,
        y_train, y_test,
        cond_prob_func, 
        classifier, 
    ...
    ## Plotting and saving some examples
    ...
    fig.tight_layout()
    check_save_fig(fig, checkpoint_dir, f"predictions_{args.classifier}_{id}")
    fig.clear()

    print(f"Plotting and saving figures: {args.data_set}, {args.classifier}, {args.recourse}")
    example_idx = [0, 2, 5, 7, 9]
    for id in example_idx:
        fig, ax = plt.subplots(1, 1, figsize=(8, 3))
        xyz, db_coords = plot_predictions_with_probs(
            classifier['model'].predict_proba, 
            X_train,
            result_dict['x_accepted'][id, :, :], 
            result_dict['y_after_recourse'][id, :],
            result_dict['predictions_after_recourse'][id, :], 
            ax, 
            title=""
        )
        fig.tight_layout()
        check_save_fig(fig, checkpoint_dir, f"predictions_{args.classifier}_{args.recourse}_{id}")
        fig.clear()

    print(f"Saving final data: {args.data_set}, {args.classifier}, {args.recourse}")
    save_synthetic_experiment_data_acceptence_probs(
        checkpoint_dir,
        result_dict, 
        args.classifier,
        args.recourse,
